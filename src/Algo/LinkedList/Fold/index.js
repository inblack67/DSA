...
class MyLinkedList {
  ...
  foldHelper(curr, index) {
    if (!curr) {
      return;
    }
    this.foldHelper(curr.next, index + 1);
    if (index > Math.floor(this.getSize() / 2)) {
      const temp = this.foldHead.next;
      this.foldHead.next = curr;
      curr.next = temp;
      this.foldHead = temp;
    } else {
      this.tail = curr;
  ...
  fold() {
    let curr = this.head;
    this.foldHead = this.head;
    this.foldHelper(curr, 0);
  }
