...
class MyBinaryTree {
  ...
  iterativeTraversals() {
    ...
    const stack: Pair[] = [];
    const pair = new Pair( this.root, 1 );
    stack.push( pair );

    while ( stack.length > 0 ) {
      const top = stack[stack.length - 1];
      if ( top.state === 1 ) {
        preorderOutput += top.someNode.data + " ";
        top.state++;
        if ( top.someNode.left ) {
          const pair = new Pair( top.someNode.left, 1 );
          stack.push( pair );
        }
      } else if ( top.state === 2 ) {
        inorderOutput += top.someNode.data + " ";
        top.state++;
        if ( top.someNode.right ) {
          const pair = new Pair( top.someNode.right, 1 );
          stack.push( pair );
        }
      }
      else {
        postorderOutput += top.someNode.data + " ";
        stack.pop();
      }
    }

    console.log( "pre", preorderOutput );
    console.log( "in", inorderOutput );
    console.log( "post", postorderOutput );
  }
